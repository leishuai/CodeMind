#!/usr/bin/env python3
"""Export a generic CodeAutonomy AgentSkill package.
...
example so another coding agent can understand CodeAutonomy as a harness-loop skill.
"""
...
def write_skill_md(out_dir: pathlib.Path) -> None:
    (out_dir / "SKILL.md").write_text(
        """---
name: codeautonomy-skill
description: "CodeAutonomy evidence-driven harness loop for coding agents. Use for coding tasks across iOS, Android, Web, and server/backend when the work needs explicit requirements, preflight, evaluator evidence, retry/replan decisions, mobile/script verification, or reusable summaries. Defaults to high automation with keeping looping implement, verify, repair."
---

# CodeAutonomy Skill

CodeAutonomy is a harness-loop skill for coding agents. It does not replace the coding agent; it gives the agent a disciplined loop:
...
or `summary-refine`; CodeAutonomy falls back to deterministic summary when the agent
...
Use CodeAutonomy when work needs any of these:
...
This skill does **not** automatically install or load the CodeAutonomy CLI every time
the skill is invoked. Skills are static instructions. At the start of every
CodeAutonomy task, the agent must actively discover whether the full CodeAutonomy runtime
is available:

1. Try `codeautonomy help`, then `automind help` as a compatibility fallback, from the target project/workspace root.
2. Try `./automind.sh help` only if the current project vendors CodeAutonomy.
...
If none of the four probes succeeds, treat CodeAutonomy as not installed and run
this guided-install loop instead of silently giving up or silently installing:

1. Tell the user the skill is available but the full CodeAutonomy CLI/runtime was not
   detected on this machine, and that full installation gives the best
   experience (CLI/runtime + `/codeautonomy` command + helpers/gates).
2. Ask the user once, explicitly, whether CodeAutonomy should install it now with the
...
3. If the user agrees (or the user already requested full-auto/no-confirmation
   ...
   or the `codeautonomy`/`automind` wrapper on `PATH`). Verify with `<AUTOMIND_CLI> help`. If the
...
5. If the user declines, do not install. Continue in skill-only mode using
   project-native verification while writing the same CodeAutonomy artifacts.
...
In skill/slash-command mode, `/codeautonomy <request>` is the canonical current-session end-to-end flow for Codex, Claude, Trae, and Trae-CN: the host coding agent remains the Planner/Generator, CodeAutonomy CLI provides scaffolding/gates/evidence helpers, and the agent must keep looping through verification and repair until required evidence passes, `ask_user` needs a human decision, or an explicit unsafe/non-recoverable stop condition is reached. `/codeautonomy --detached <request>` (or the equivalent `automind ask "<request>" <agent>`) is the detached-CLI variant; use it only for explicit detached/background requests. `/codeautonomy ask <request>` is kept as an alias for `/codeautonomy <request>` and behaves identically in the current session. In detached Codex/Claude/Trae/Trae-CN mode, Planner/Generator may reuse one task-local primary CLI session; Evaluator must still be a fresh isolated invocation.

Think of `/codeautonomy <request>` as structured natural language for the current model: "use the CodeAutonomy skill/protocol in this conversation; call CodeAutonomy CLI helpers when useful; run the harness loop end-to-end; do not start a detached agent unless explicitly requested."
...
When only this skill package is available, first suggest the single full install command (`curl -fsSL https://raw.githubusercontent.com/leishuai/CodeAutonomy/main/install-curl.sh | bash`). If installation is not allowed, follow the same workflow manually and use project-native build/test/device commands for evidence.
...
Best user experience is **full CodeAutonomy installation** with the single public install command: `curl -fsSL https://raw.githubusercontent.com/leishuai/CodeAutonomy/main/install-curl.sh | bash`. It installs the CodeAutonomy CLI/runtime, this skill, and the `/codeautonomy` command for supported coding agents. In that mode, the skill gives the agent the workflow and prompt protocol, while the full CodeAutonomy checkout provides `automind` / `./automind.sh` plus orchestrator/scripts/adapters.

When a full CodeAutonomy runtime is available, agents may use:

1. `codeautonomy`, `automind`, or `./automind.sh` helpers/gates first (`scaffold`, `status`, `workflow-check`, `context-pack`, `script-command`, `completion-check`, `trace`, `process-check`, `summary`, `summary-refine`);
2. documented scripts/adapters from the full checkout only when the command catalog says direct script use is appropriate.

Important workspace rule: CodeAutonomy runtime and user workspace are separate. The public installer puts the runtime checkout at `$HOME/.automind/automind` by default (`AUTOMIND_HOME` may override it) and creates `$HOME/.local/bin/codeautonomy` plus the `$HOME/.local/bin/automind` compatibility wrapper (`AUTOMIND_BIN_DIR` may override it). Run CodeAutonomy CLI commands from the target project/workspace root. Task artifacts are written to `$AUTOMIND_WORKSPACE_ROOT/.automind/tasks` when set, otherwise to the current working directory's `.automind/tasks`. If the agent is not currently in the target project root, set `AUTOMIND_WORKSPACE_ROOT=/path/to/project` before running `automind ...`; do not let an installed CodeAutonomy checkout become the task workspace. Do not copy developer-machine absolute paths from old logs; resolve runtime resources relative to the current runtime root. See `docs/references/installation-runtime.md`.

## Mandatory startup read protocol

This is not a reference-only list. At the start of every CodeAutonomy task, before
...
5. `docs/references/command-script-catalog.md` — choose CodeAutonomy CLI helpers or direct scripts safely.
...
- Execute the Mandatory startup read protocol at the beginning of each CodeAutonomy task; do not treat the doc list as optional background reading.
...
- At the start of each CodeAutonomy task, discover `<AUTOMIND_CLI>` with `codeautonomy help`, `automind help`, project-local `./automind.sh help`, `$HOME/.automind/automind/automind.sh help`, or `$AUTOMIND_HOME/automind.sh help`. Run the chosen CLI from the target project/workspace root, or set `AUTOMIND_WORKSPACE_ROOT=/path/to/project`; if no CLI is available, recommend full install before falling back to skill-only mode.
...
- In skill/command mode, use the `checklist[]` / `checkboxMarkdown[]` returned by `automind-workflow-state.json` / `phase-gate` as the native TODO/checkbox plan. Complete required checklist items one by one with the coding agent's normal ability to read, edit, test, and write artifacts; run a CLI command only when that checklist item explicitly names a CodeAutonomy helper/gate. Review `Reuse.md` and relevant `phase-reuse/<phase>.md` when the checklist asks for them, as guidance only. Then rerun `phase-gate`.
...
- Run `<AUTOMIND_CLI> completion-check <task-code>` before claiming Finish; if it fails, route back by `evaluation.json.nextAction` or replan. If it passes, prefer `<AUTOMIND_CLI> summary <task-code> --ai <agent>` or `<AUTOMIND_CLI> summary-refine <task-code> <agent>`, fall back to `<AUTOMIND_CLI> summary <task-code>` when no agent is available, then run `<AUTOMIND_CLI> record-check <task-code>` and `<AUTOMIND_CLI> report <task-code>` before final handoff. The HTML report should show `<task> CodeAutonomy Report`, Test Results with per-TC evidence/screenshots/logs, and Summary / Knowledge Deposition.
...
- If a full CodeAutonomy CLI is available in skill/slash-command mode, prefer `automind scaffold "<request>"` to create the task artifacts without launching another agent process. Before running it, ensure the shell cwd is the target project root or set `AUTOMIND_WORKSPACE_ROOT=/path/to/project`; verify the printed `TASK_DIR` is under that target project.
...
## Updating CodeAutonomy itself

When the user asks to update, upgrade, refresh, reinstall, or sync CodeAutonomy itself,
...
- "更新 CodeAutonomy"
- "升级一下 automind"
- "refresh CodeAutonomy runtime"
- "同步最新 CodeAutonomy skill/command"

This updates the CodeAutonomy runtime, CLI wrappers, skill package, and `/codeautonomy`
slash-command integrations. Update-only intent is **not** a CodeAutonomy task:
...
def write_readme(out_dir: pathlib.Path, copied: dict[str, list[str]], include_internal: bool = False, include_private: bool = False) -> None:
    ...
    (out_dir / "README.md").write_text(
        f"""# CodeAutonomy Skill Export

Generated at: {datetime.now().isoformat(timespec='seconds')}

This package is a generic CodeAutonomy skill bundle for coding agents. It packages the stable workflow docs, prompt templates, schemas, selected summary knowledge, and a minimal artifact-shape example. The recommended setup is full CodeAutonomy installation via `curl -fsSL https://raw.githubusercontent.com/leishuai/CodeAutonomy/main/install-curl.sh | bash`: the CLI/runtime provides `automind` commands and scripts/adapters, while this skill teaches agents how to use them correctly. The public skill package itself does not package the executable CodeAutonomy runtime.{public_note}

## What CodeAutonomy gives a coding agent
...
5. Use `docs/references/test-design-guide.md` for detailed testcase/runbook examples, `docs/references/command-script-catalog.md` before choosing which CodeAutonomy command or script adapter to run, and `docs/references/verification-flow.md` before platform/device verification.
...
The catalog in `docs/references/command-script-catalog.md` names the commands and scripts behind CodeAutonomy capabilities. The recommended user setup is full CodeAutonomy installation, where this skill may call `automind` / `./automind.sh` and, when documented, full-checkout scripts/adapters. This public skill export intentionally does **not** bundle those executable runtime scripts or platform adapters; it is a protocol/docs/templates/schemas/examples package. If no full CodeAutonomy CLI checkout or installed `automind` wrapper is available, first suggest the single full install command (`curl -fsSL https://raw.githubusercontent.com/leishuai/CodeAutonomy/main/install-curl.sh | bash`). If installation is not allowed, follow `docs/workflow.md` manually and use the target project's own build/test/device tools to collect equivalent evidence.

Mobile/visual automation helper packages are lazy/local. The installer and this skill export do not install Android/iOS/visual helper kits by default. In full runtime mode, required low-risk Python helpers may be auto-created/repaired during preflight/evaluation via `automind setup-automation-tools android`, `automind setup-automation-tools ios`, or `automind setup-automation-tools visual`. These commands use the CodeAutonomy runtime `requirements/*.txt`, create local Python virtualenvs in the target workspace, retry transient network/DNS package-index failures once with explicit logs, and must not install system SDKs, signing material, OCR engines, browser drivers, device trust settings, or privileged services.
...
CodeAutonomy turns AI coding from "write code and hope" into a recoverable, evidence-driven engineering loop.
""",
        encoding="utf-8",
    )
...
This exported example shows the expected CodeAutonomy task artifact shape without bundling any local `.automind/tasks/` runtime folders.

In a real CodeAutonomy checkout, create fresh local runtime evidence with:
...
""",
    ...
    (demo / "artifact-shape.md").write_text(
        """# CodeAutonomy Artifact Shape

CodeAutonomy task runtime folders are created locally while the user works. The public skill export only documents the shape:
...
def looks_like_legacy_automind_skill(path: pathlib.Path) -> bool:
    """Return true only for old CodeAutonomy-generated skill folders safe to remove.
    ...
    try:
        ...
        return False
    return (
        ("name: automind" in text or "name: automind-skill" in text)
        and ("CodeAutonomy is a harness-loop skill" in text or "AutoMind is a harness-loop skill" in text)
    )
...
def main() -> int:
    ...
    parser.add_argument("out_dir", nargs="?", default=str(pathlib.Path.home() / "Downloads" / "codeautonomy-skill"))
    ...
    parser.add_argument("--install-name", default="codeautonomy-skill", help="Folder name to use when installing into an agent skill directory.")
