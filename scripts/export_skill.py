#!/usr/bin/env python3
"""Export a generic CodeMind AgentSkill package.
...
example so another coding agent can understand CodeMind as a harness-loop skill.
"""
...
INTERNAL_OPTIONAL = [
    "internal/README.md",
    "internal/lark-channel-design.md",
    "internal/lark-oncall-feedback-bot-design.md",
    "internal/orchestrator-multi-agent-design.md",
...
def write_skill_md(out_dir: pathlib.Path, skill_name: str = "codemind-skill") -> None:
    skill_text = """---
name: __CODEMIND_SKILL_NAME__
description: "CodeMind evidence-driven harness loop for coding agents. Use for coding tasks across iOS, Android, Web, and server/backend when the work needs explicit requirements, preflight, evaluator evidence, retry/replan decisions, mobile/script verification, or reusable summaries. Defaults to high automation with keeping looping implement, verify, repair."
---

# CodeMind Skill

CodeMind is a harness-loop skill for coding agents. It does not replace the coding agent; it gives the agent a disciplined loop:
...
or `summary-refine`; CodeMind falls back to deterministic summary when the agent
...
Use CodeMind when work needs any of these:
...
This skill does **not** automatically install or load the CodeMind CLI every time
the skill is invoked. Skills are static instructions. At the start of every
CodeMind task, the agent must actively discover whether the full CodeMind runtime
is available:

1. Try `codemind help` from the target project/workspace root.
2. Try the legacy `automind help` alias.
3. Try `./automind.sh help` only if the current project vendors CodeMind.
4. Try the default install path: `$HOME/.automind/automind/automind.sh help`.
5. Try `$AUTOMIND_HOME/automind.sh help` when `AUTOMIND_HOME` is set.
...
If none of the four probes succeeds, treat CodeMind as not installed and run
this guided-install loop instead of silently giving up or silently installing:

1. Tell the user the skill is available but the full CodeMind CLI/runtime was not
   detected on this machine, and that full installation gives the best
   experience (CLI/runtime + `/codemind` command + helpers/gates).
2. Ask the user once, explicitly, whether CodeMind should install it now with the
...
3. If the user agrees (or the user already requested full-auto/no-confirmation
   ...
   newly installed `<AUTOMIND_CLI>` (typically the `codemind` wrapper on `PATH`
   or `$HOME/.automind/automind/automind.sh`). Verify with `<AUTOMIND_CLI> help`. If the
...
5. If the user declines, do not install. Continue in skill-only mode using
   project-native verification while writing the same CodeMind artifacts.
...
In skill/slash-command mode, `/codemind <request>` is the canonical current-session end-to-end flow for Codex, Claude, Trae, and Trae-CN: the host coding agent remains the Planner/Generator, CodeMind CLI provides scaffolding/gates/evidence helpers, and the agent must keep looping through verification and repair until required evidence passes, `ask_user` needs a human decision, or an explicit unsafe/non-recoverable stop condition is reached. `/codemind --detached <request>` (or the equivalent `codemind ask "<request>" <agent>`) is the detached-CLI variant; use it only for explicit detached/background requests. `/automind` remains a compatibility alias. In detached Codex/Claude/Trae/Trae-CN mode, Planner/Generator may reuse one task-local primary CLI session; Evaluator must still be a fresh isolated invocation.

Think of `/codemind <request>` as structured natural language for the current model: "use the CodeMind skill/protocol in this conversation; call CodeMind CLI helpers when useful; run the harness loop end-to-end; do not start a detached agent unless explicitly requested."
...
1. `codemind`, legacy `automind`, or `./automind.sh` helpers/gates first (`scaffold`, `status`, `workflow-check`, `context-pack`, `script-command`, `completion-check`, `trace`, `process-check`, `summary`, `summary-refine`);
2. documented scripts/adapters from the full checkout only when the command catalog says direct script use is appropriate.

Important workspace rule: CodeMind runtime and user workspace are separate. The public installer keeps its compatibility runtime path at `$HOME/.automind/automind` by default (`AUTOMIND_HOME` may override it) and creates the canonical wrapper at `$HOME/.local/bin/codemind` (`AUTOMIND_BIN_DIR` may override it). Run CodeMind CLI commands from the target project/workspace root. Task artifacts are written to `$AUTOMIND_WORKSPACE_ROOT/.automind/tasks` when set, otherwise to the current working directory's `.automind/tasks`. If the agent is not currently in the target project root, set `AUTOMIND_WORKSPACE_ROOT=/path/to/project` before running `codemind ...`; do not let an installed CodeMind checkout become the task workspace. Do not copy developer-machine absolute paths from old logs; resolve runtime resources relative to the current runtime root. See `docs/references/installation-runtime.md`.

## Mandatory startup read protocol

This is not a reference-only list. At the start of every CodeMind task, before
...
5. `docs/references/command-script-catalog.md` — choose CodeMind CLI helpers or direct scripts safely.
...
- Execute the Mandatory startup read protocol at the beginning of each CodeMind task; do not treat the doc list as optional background reading.
...
- At the start of each CodeMind task, discover `<AUTOMIND_CLI>` with `codemind help`, legacy `automind help`, project-local `./automind.sh help`, `$HOME/.automind/automind/automind.sh help`, or `$AUTOMIND_HOME/automind.sh help`. Run the chosen CLI from the target project/workspace root, or set `AUTOMIND_WORKSPACE_ROOT=/path/to/project`; if no CLI is available, recommend full install before falling back to skill-only mode.
...
- In skill/command mode, use the `checklist[]` / `checkboxMarkdown[]` returned by `automind-workflow-state.json` / `phase-gate` as the native TODO/checkbox plan. Complete required checklist items one by one with the coding agent's normal ability to read, edit, test, and write artifacts; run a CLI command only when that checklist item explicitly names a CodeMind helper/gate. Review `Reuse.md` and relevant `phase-reuse/<phase>.md` when the checklist asks for them, as guidance only. Then rerun `phase-gate`.
...
- Run `<AUTOMIND_CLI> completion-check <task-code>` before claiming Finish; if it fails, route back by `evaluation.json.nextAction` or replan. If it passes, prefer `<AUTOMIND_CLI> summary <task-code> --ai <agent>` or `<AUTOMIND_CLI> summary-refine <task-code> <agent>`, fall back to `<AUTOMIND_CLI> summary <task-code>` when no agent is available, then run `<AUTOMIND_CLI> record-check <task-code>` and `<AUTOMIND_CLI> report <task-code>` before final handoff. The HTML report should show `<task> CodeMind Report`, Test Results with per-TC evidence/screenshots/logs, and Summary / Knowledge Deposition.
...
- If a full CodeMind CLI is available in skill/slash-command mode, prefer `codemind scaffold "<request>"` to create the task artifacts without launching another agent process. Before running it, ensure the shell cwd is the target project root or set `AUTOMIND_WORKSPACE_ROOT=/path/to/project`; verify the printed `TASK_DIR` is under that target project.
...
## Updating CodeMind itself

When the user asks to update, upgrade, refresh, reinstall, or sync CodeMind itself,
...
- "更新 CodeMind"
- "升级一下 automind"
- "refresh CodeMind runtime"
- "同步最新 CodeMind skill/command"

This updates the CodeMind runtime, CLI wrapper, skill package, and `/codemind`
slash-command integrations. Update-only intent is **not** a CodeMind task:
...
"""
    (out_dir / "SKILL.md").write_text(
        skill_text.replace("__CODEMIND_SKILL_NAME__", skill_name, 1),
        encoding="utf-8",
    )
...
3. Discover `<AUTOMIND_CLI>` by trying `codemind help`, legacy `automind help`, project-local `./automind.sh help`, `$HOME/.automind/automind/automind.sh help`, then `$AUTOMIND_HOME/automind.sh help`. If none works, recommend the full install command before falling back to skill-only mode. Treat the chosen CLI path as runtime/executable location only; task artifacts still belong under the target workspace.
4. Read `docs/README.md` for the doc map.
5. Use `docs/references/test-design-guide.md` for detailed testcase/runbook examples, `docs/references/command-script-catalog.md` before choosing which CodeMind command or script adapter to run, and `docs/references/verification-flow.md` before platform/device verification.
...
12. Use `templates/evaluator_prompt.md` for Verify; Evaluator must write `Validation.md`, `evaluation.json`, evidence, and `nextAction`. Prefer measurable visual evidence first. When screenshots/images exist and the host model supports image understanding, use `templates/visual_review_prompt.md` as a supplementary semantic review and save the result under `logs/iter-N/ai-visual-review.json`. When the host cannot inspect images, prefer deterministic fallback (`codemind visual-inspect`, screenshot diff, OCR, bounds/hierarchy, project-native snapshot/layout tests) before asking the user. If none can prove a visual claim, show captured screenshot(s) to the user with `nextAction=ask_user`; explicit user confirmation is the final fallback evidence.
...
Web/client/server project dependency setup remains project-native. Use `codemind dependency-check [task-code] [iteration]` only as an optional read-only planner/preflight report when dependency setup is unclear, then run the repository's lockfile/documented commands when required by TestCases/Plan and record evidence.

## Product idea in one line

CodeMind turns AI coding from "write code and hope" into a recoverable, evidence-driven engineering loop.
""",
        encoding="utf-8",
    )
...
This exported example shows the expected CodeMind task artifact shape without bundling any local `.automind/tasks/` runtime folders.

In a real CodeMind checkout, create fresh local runtime evidence with:
...
""",
    ...
    (demo / "artifact-shape.md").write_text(
        """# CodeMind Artifact Shape

CodeMind task runtime folders are created locally while the user works. The public skill export only documents the shape:
...
def looks_like_legacy_automind_skill(path: pathlib.Path) -> bool:
    """Return true only for old CodeMind-generated skill folders safe to remove.
    ...
    try:
        ...
        return False
    return (
        "name: automind" in text
        and (
            "AutoMind is a harness-loop skill" in text
            or "CodeMind is a harness-loop skill" in text
        )
    )
...
def install_skill(
    ...
    agent: str,
    install_name: str,
    compatibility_names: list[str] | None = None,
) -> dict:
    if agent == "none":
    ...
    for kind, target in selected:
        ...
        for compatibility_name in compatibility_names or []:
            compatibility_target = target.parent / compatibility_name
            copy_tree_clean(out_dir, compatibility_target)
            compatibility_skill = compatibility_target / "SKILL.md"
            compatibility_text = compatibility_skill.read_text(encoding="utf-8")
            compatibility_text = compatibility_text.replace(
                f"name: {install_name}",
                f"name: {compatibility_name}",
                1,
            )
            compatibility_skill.write_text(compatibility_text, encoding="utf-8")
            installed.append({
                "kind": kind,
                "path": display_path(compatibility_target),
...
def main() -> int:
    ...
    parser.add_argument("out_dir", nargs="?", default=str(pathlib.Path.home() / "Downloads" / "codemind-skill"))
    ...
    parser.add_argument("--install-name", default="codemind-skill", help="Folder name to use when installing into an agent skill directory.")
    parser.add_argument("--no-legacy-alias", action="store_true", help="Do not install the legacy automind-skill alias.")
    ...
    write_skill_md(out_dir, args.install_name)
    ...
    compatibility_names = (
        ["automind-skill"]
        if not args.no_legacy_alias and args.install_name == "codemind-skill"
        else []
    )
    ...
    manifest["compatibilityAliases"] = compatibility_names
