...
if [[ ! -x "$PY" ]]; then
  echo "[AutoMind] Android tools python not found: $PY" >&2
  echo "[AutoMind] Expected project venv: $WORKSPACE_ROOT/.venv-android-tools/" >&2
  exit 2
fi
...
if missing:
    print("[AutoMind] Missing Android python modules: " + ", ".join(missing), file=sys.stderr)
    sys.exit(2)
PY
...
- **AC-002**: Assert `AutoMind Android Harness Demo` and `Probe state: Idle`, tap `probe_button` / `Run Probe`, then assert `Probe state: Completed`.
  - Verification method: android-probe-flow / TC-F01
MD
...
cat > "$FLOW" <<JSON
{
  ...
  "steps": [
    ...
    {"type": "assert_text", "name": "assert title", "text": "AutoMind Android Harness Demo"},
...
if [[ "$ITER1_CODE" -eq 0 ]]; then
  echo "[AutoMind] Expected iter-1 dynamic probe-flow to fail, but it passed" >&2
  exit 1
fi
