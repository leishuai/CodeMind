#!/usr/bin/env python3
"""iOS physical screenshot evaluator for AutoMind.
...
def build_ios_tools_ask(reason: str) -> dict[str, Any]:
    return {
        "question": "iOS screenshot helper tools are still unavailable after AutoMind tried local helper setup. What should happen next?",
        "reason": reason + " AutoMind can auto-create .venv-ios-tools for low-risk Python helper packages, but pymobiledevice3 is still unavailable. This may require fixing network/proxy/Python/pip, or skipping physical screenshot evidence.",
        "options": [
...
def main() -> int:
    ...
    parser.add_argument("--device-id", default=None, help="xcodebuild/traditional UDID for pymobiledevice3 --tunnel. If omitted, AutoMind reads runtime-state iosApp/iosDevice fields.")
    ...
    if ok:
        ...
        if not device_id:
            ...
            ask = {
                "question": "Which iOS device id should AutoMind use for screenshot capture?",
            ...
            next_action = "ask_user"
            ask = {
                "question": "Has pymobiledevice3 tunneld been started, or may AutoMind temporarily start it with sudo?",
