#!/usr/bin/env python3
"""Suggest/apply minimal probe-flow repairs from AutoMind evaluation artifacts.
...
def load_flow(task_dir: Path) -> tuple[Path, dict[str, Any]]:
    ...
    if legacy_path.exists():
        print(
            f"[AutoMind] WARN: `{legacy_path.name}` is deprecated; rename to `probe-flow.android.json` (Android) or `probe-flow.ios.json` (iOS).",
            file=sys.stderr,
        )
...
def main() -> int:
    ...
    ap.add_argument("--runtime-root", default=None, help="AutoMind runtime checkout. Defaults to this script's checkout.")
