...
def build_ios_tools_ask(reason: str) -> dict[str, Any]:
    return {
        "question": "Optional iOS screenshot helper tools are still unavailable after AutoMind tried local helper setup. What should happen next?",
        "reason": reason + " AutoMind can auto-create .venv-ios-tools for low-risk Python helper packages, but pymobiledevice3 is still unavailable. This may require fixing network/proxy/Python/pip, or continuing without screenshot evidence.",
        "options": [
...
def main() -> int:
    ...
    if screenshot_enabled and screenshot.get("category") == "tool_missing":
        evaluation.setdefault("warnings", []).append({
            ...
            "category": "tool_missing",
            "reason": "AutoMind tried to create/repair .venv-ios-tools from requirements/ios-tools.txt, but screenshot helper tools are still unavailable: " + screenshot.get("reason", "iOS screenshot helper tools missing"),
