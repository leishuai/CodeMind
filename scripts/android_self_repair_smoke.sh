...
if [[ ! -d "$ANDROID_VENV" ]]; then
  echo "[CodeMind] Android tool venv not found: $ANDROID_VENV" >&2
  echo "[CodeMind] Create it with: python3 -m venv $ANDROID_VENV && source $ANDROID_VENV/bin/activate && pip install adbutils uiautomator2" >&2
  exit 2
fi
...
python3 "$ROOT/scripts/android_app_harness_probe.py" \
  ...
  --initial-text 'CodeMind Android Harness Demo' \
...
if [[ "$ITER1_CODE" -eq 0 ]]; then
  echo "[CodeMind] Expected iter-1 to fail, but it passed" >&2
  exit 1
fi
