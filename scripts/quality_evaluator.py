#!/usr/bin/env python3
"""Lightweight CodeMind quality evaluator.
...
def should_scan_text_evidence(path: pathlib.Path, task_dir: pathlib.Path) -> bool:
    ...
    Quality checks should judge the current functional evidence, not CodeMind's
    ...
    def add_path(raw: Any) -> None:
        ...
        if not p.is_absolute():
            if raw_path.startswith(".automind/") or raw_path.startswith("logs/"):
                # CodeMind artifacts use both workspace-relative
...
def quality_checks(root: pathlib.Path, task_dir: pathlib.Path, log_dir: pathlib.Path) -> tuple[list[dict], dict]:
    ...
    if changed_files:
        ...
        checks.append({
            ...
            "result": "pass",
            "actual": 0,
            "reason": "No git diff detected in CodeMind repo; architecture scope check has no changed files to review.",
            "triageSource": "code_deterministic",
            "needsModelReview": False,
        })
...
def main() -> int:
    ...
    parser.add_argument("--root", default=".", help="CodeMind repo root. Default: current directory.")
