...
AutoMind is agent-agnostic at its core. Codex, Claude Code, Trae/Trae-CN, Cursor, or future agents should all consume the same AutoMind task artifacts:
...
AutoMind loop
  ...
  -> AutoMind evaluation / runtime-state
```
...
3. **Working directory policy**
   ...
   - AutoMind must run agent/project subprocesses with the target workspace root as cwd. `AUTOMIND_ROOT` is only the installed runtime/scripts/templates root.
...
Adapters should not own AutoMind's product logic:
...
Those stay in AutoMind core so every agent follows the same loop.
...
AutoMind currently supports these thin CLI adapters:
...
| `trae` / `trae-cn` | `traecli` | Planner/Generator: `traecli -p <prompt> --session-id/--resume <uuid> --allowed-tool ... --yolo --json`; Evaluator: fresh `traecli -p ... --yolo --json` | Both adapter names map to the same Trae CLI runtime. `trae-cn` is accepted as a detached CLI alias and also matches the Trae-CN skill/command install target. Detached/non-TTY command mode defaults to YOLO/tool auto-approval for high automation while AutoMind prompt-level unsafe-action guards still require `ask_user`. |

`auto` is the default detached CLI selection mode for `ask`, `plan`, and `resume`. It tries `codex`, then `claude`, then `trae`/`trae-cn` (`traecli`); the first adapter whose binary and `--version` probe pass is used for the actual run. If none pass, AutoMind returns a full preflight diagnostic and recommends installing/configuring a supported CLI or using current-session `scaffold`/`/automind` mode.

## Evaluator isolation across coding agents

AutoMind should not assume every tool has a native "subagent" primitive. The portable guarantee is stricter and simpler:
...
4. **Policy validation gate**: before launching an agent Evaluator, AutoMind validates the pack:
...
AutoMind must keep raw evidence on disk, not in the coding-agent conversation. Context packs use a tiered policy: core contract files such as `Requirements.md`, `TestCases.md`, `Plan.md`, and `Brainstorm.md` get much larger/full excerpts so task intent is not lost; history/evidence files such as `Delivery.md` and `Validation.md` use structured excerpts (headings, key result/evidence lines, latest sections) when large; build logs, probe logs, screenshots, DB dumps, and raw agent transcripts remain authoritative artifacts under the task directory and are reached through `log-digest.md` plus targeted `grep`/`tail`/line-range reads. If an agent hits `agent_context_overflow`, AutoMind should clear the saturated primary session and resume through a fresh session using durable task artifacts as the handoff.
...
5. **Explicit detached CLI ownership**
   - Run `./automind.sh ask ...` or `./automind.sh resume ...` only when the user wants AutoMind to own a background/detached loop through an adapter.
...
- "Codex may require a trusted git workspace; AutoMind is passing `--skip-git-repo-check`."

Bad additions:

- Duplicating the whole Generator/Evaluator prompt per agent.
- Changing the AutoMind task file protocol per agent.
- Letting one adapter silently skip evidence or structured evaluation.

## Failure mapping

All adapter failures should normalize into AutoMind categories:

| Adapter symptom | AutoMind category | Typical nextAction |
|---|---|---|
...
| CLI process exceeds AutoMind timeout | `agent_timeout` | safe auto-recovery: keep artifacts and retry/resume without `ask_user`; increasing timeout is also safe when bounded and local. Default detached agent timeout is 43200 seconds. |
...
CLI/TUI-owned loops also wrap `run_harness_loop` with a bounded safe auto-resume supervisor. When Generator/Evaluator writes `evaluation.autoRecovery` for `agent_unavailable`, `agent_timeout`, `agent_stalled_no_output`, or `agent_context_overflow`, AutoMind re-enters the same task automatically instead of asking the user to run `resume`. For `agent_context_overflow`, it clears the saturated primary Planner/Generator session and retries from durable task artifacts in a fresh primary session. The default outer auto-resume limit is 3 (`AUTOMIND_SAFE_AUTO_RESUME_MAX`) to avoid infinite loops.
...
If an adapter needs more than ~100 lines or starts changing workflow semantics, it is probably not an adapter anymore. Move shared behavior back into AutoMind core.

## Using AutoMind as a skill or command in coding agents

There are three integration modes. Prefer the skill package for agents that support reusable skills. Use slash-command export for agents that expose `/command` style entrypoints. Use repo-local command mode when the agent is working directly inside the AutoMind repository or a project that vendors AutoMind.

```text
Mode A: Skill package
  AutoMind repo -> ./automind.sh export-skill <skill-dir> -> install/import into agent skill system

Mode B: Slash-command package
  AutoMind repo -> ./automind.sh export-command <command-dir> -> install/import /automind command

Mode C: Command/project tool
  Agent works in a repo that has AutoMind -> run ./automind.sh commands directly
```
...
```bash
cd /path/to/automind
./automind.sh export-skill /tmp/automind-skill
```
...
./automind.sh export-skill /tmp/automind-skill --install auto

# Install for Claude user-level skill folder
./automind.sh export-skill --install claude

# Same, but keep/export a copy at a specific path too
./automind.sh export-skill /tmp/automind-skill --install claude
...
AutoMind does not silently modify agent configuration without `--install`. Trae does not support PreToolUse hooks; use the skill plus project `AGENTS.md` rules for always-on behavior.
...
```text
Use the AutoMind skill. For this task, first execute the mandatory startup read
...
1. Put the exported AutoMind skill folder inside the working project, for example `.agent-skills/automind-skill/`.
2. Add a short project instruction telling Codex to read `.agent-skills/automind-skill/SKILL.md` before long-running tasks.
3. If AutoMind itself is available as a command, tell Codex to use command mode below.

Suggested prompt:

```text
Use .agent-skills/automind-skill as the AutoMind skill. Read SKILL.md and follow the
AutoMind harness loop: requirements, preflight, generator/evaluator evidence,
...
```bash
./automind.sh export-skill --install trae
# -> ~/.trae/skills/automind-skill

./automind.sh export-skill --install trae-cn
# -> ~/.trae-cn/skills/automind-skill
```

Trae does not support PreToolUse hooks. Use the exported skill plus project `AGENTS.md` rules as the always-on mechanism. Repo-local command mode is still useful when you want the agent to run AutoMind directly from a checkout.

Suggested prompt for repo-local use:

```text
Use the AutoMind skill in .agent-skills/automind-skill. Do not rely on interactive
questions unless AutoMind asks for a human decision. Produce structured evidence
and evaluation output.
```

### Mode B: exported slash-command package

Some coding agents expose slash commands such as `/graphify`. AutoMind supports the same style through `export-command`: `/automind` is a thin entrypoint that makes the host agent follow the AutoMind skill/workflow protocol. When an AutoMind CLI is available, the command uses it for deterministic helpers and gates; it does not start a detached agent loop unless the user explicitly asks for detached mode.
...
|---|---|---|
| Claude | `~/.claude/commands/automind.md` | Claude-style markdown slash command. |
| Codex | `~/.codex/commands/automind.md` | Codex-style markdown command entrypoint; pairs with AutoMind skill at `~/.codex/skills/automind-skill`. |
| Trae | `~/.trae/commands/automind.md` | Follows the same user-level convention as graphify-style Trae installs, using `commands/` for slash commands. |
| Trae-CN | `~/.trae-cn/commands/automind.md` | Domestic Trae/Trae-CN variant. |

The command file is intentionally small. It should not duplicate all AutoMind docs; it routes the agent to the AutoMind CLI, `SKILL.md`, and the workflow docs so the CLI, skill, and slash command keep one source of truth. The generated command resolves the CLI in this order: project-local `./automind.sh`, `$AUTOMIND_HOME/automind.sh`, then `automind.sh`/`automind` on `PATH`.

Default slash-command semantics:

- `/automind ask <request>` is equivalent to `/automind <request>`.
- Both mean: use the current host-agent session as Planner/Generator and use AutoMind CLI helpers/gates such as `scaffold`, `workflow-check` (refresh/validate `workflow.json`), `phase-gate` (refresh/read `automind-workflow-state.json`, checklist, and missing/stale generator/evaluator phase reuse at delivery/evaluation handoff), `context-pack`, and `completion-check`.
- They do **not** call `automind ask "<request>" [auto|codex|claude|trae]` by default, because that command starts an AutoMind-owned detached loop and launches a separate agent CLI process/session.
- Use `/automind detached ask <request>` or `/automind cli-ask <request>` only when a separate background CLI loop is intended.

Example usage after install:

```text
/automind ask Add login smoke validation for this app
/automind Add login smoke validation for this app
/automind resume login_smoke_05111530
/automind status login_smoke_05111530
/automind verify login_smoke_05111530
/automind detached ask Add login smoke validation for this app
```

### Mode C: command/project tool

When the agent is operating inside the AutoMind repo, or inside a project that has AutoMind installed, use `./automind.sh` directly.
...
## Should projects mention AutoMind in AGENTS.md?

Yes. If AutoMind is installed or vendored into a project, add a short `AGENTS.md` section so any coding agent knows when and how to use it.
...
Use AutoMind for non-trivial coding tasks that need explicit requirements,
...
If AutoMind is available as a command, use:
...
If AutoMind is installed as a skill, execute its mandatory startup read protocol
...
Keep the project `AGENTS.md` short. It should point agents to AutoMind, not duplicate all AutoMind docs.


## Shared session, TUI, and observability

Adapters should treat session/TUI/trace/process-eval behavior as a shared
AutoMind protocol, not adapter-specific state. See
