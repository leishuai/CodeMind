...
iOS-specific verification commands and flows for CodeMind. This is loaded on
...
8. **Use human-assisted evidence capture after automated paths.** The user's
   physical action is not proof unless CodeMind captures machine-checkable
...
Preflight/evaluator may run this automatically when screenshot/app-smoke requires the helper and it is missing; users can also run it up front. It creates `.venv-ios-tools` in the target workspace and installs packages from the CodeMind runtime `requirements/ios-tools.txt`; it does not install Xcode, change signing, start `tunneld`, or manipulate devices. `tunneld`/sudo remains a separate human-confirmed step when needed.
...
CodeMind can operate iOS apps through XCUITest/probe-flow. Do not describe iOS
...
`.xcresult`/logs is CodeMind's own verification work. Encode and run them through
...
exactly what CodeMind detected and the single physical action needed.
...
4. **Temporary project edit to obtain a real-device UI runner.** If tiers 1-3
   cannot run and the blocker is fixable, CodeMind may add/enable a UI test
...
3. **Human-assisted real-device evidence capture.** Only after automated
   ...
   while CodeMind captures screenshots/screen recording, syslog/console/runtime
