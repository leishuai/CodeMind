...
Use this checklist when AutoMind is running as a skill or slash command, where the host coding agent drives the loop instead of the CLI/TUI orchestrator.
...
- `command` is optional. Use it only when the item needs an AutoMind CLI helper/gate such as `workflow-check`, `phase-gate`, `context-pack`, `completion-check`, `summary`, or `report`.
