# CodeMind Command and Script Catalog

This is the canonical map from **agent/user intent** to CodeMind commands and the scripts behind them.

Use this file when you need to answer:

- Which CodeMind command should I run for this task?
...
Prefer the CodeMind CLI wrapper. Direct script invocation is an implementation detail unless this catalog explicitly says it is a direct adapter.
...
1. Canonical installed wrapper on `PATH`: `codemind`.
2. Legacy installed wrapper on `PATH`: `automind`.
3. Project-local/vendored CodeMind checkout: `./automind.sh` only when the target project contains CodeMind.
4. Default full install path: `$HOME/.automind/automind/automind.sh`.
5. Explicit home override: `$AUTOMIND_HOME/automind.sh`.
5. Last resort in repo internals: `python3 orchestrator/main.py ...` or `python3 scripts/<adapter>.py ...`

Examples in this repository usually show `./automind.sh`. Installed users should use `codemind`; `automind` remains a compatibility alias. Always run CodeMind commands from the target project root; otherwise set `AUTOMIND_WORKSPACE_ROOT=/path/to/project` so `.automind/tasks` is created in the project, not in the CodeMind runtime checkout.

If you are reading this inside an exported skill-only package, executable runtime scripts and platform adapters are not bundled. The recommended setup is full CodeMind installation; then the skill may use the installed CLI and documented full-checkout scripts/adapters. Prefer the CLI wrapper first. Direct script use is allowed only when this catalog marks it as an adapter/debug/direct path. If no CLI is available, first suggest the single full install command (`curl -fsSL https://github.com/leishuai/CodeMind/raw/refs/heads/main/install-curl.sh | bash`). If installation is not allowed, follow `docs/workflow.md` manually and use the target project's own build/test tools for evidence.

For slash-command integrations, treat `/codemind <request>` as current-session natural language that invokes the CodeMind skill/protocol. It should normally map to helper/gate commands such as `scaffold`, `workflow-check`, `context-pack`, and `completion-check`, not to detached `ask`. In Skill/slash-command mode, JSON outputs are part of the command contract: `workflow-check` refreshes sidecars and `workflow.json`; `evaluation.json.nextAction`, `completion-report.json`, and `VerificationLedger.json` drive the next step.
...
|---|---|---|---|---|---|
| Check local CodeMind basics | `<AUTOMIND_CLI> init` | `automind.sh` | Setup | terminal output, `.automind/` dirs | Does not install mobile SDKs. It only checks optional tools and creates local CodeMind directories. |
| Update CodeMind itself | `<AUTOMIND_CLI> update` | `orchestrator/main.py` → bundled `install-curl.sh` | Maintenance | updated runtime, CLI wrapper, skill package, `/codemind` command | Prefer this when the user asks to update/upgrade/refresh/sync CodeMind. It is not a task workflow: do not scaffold a new task for update-only intent. If the runtime is too old to have `update`, use the documented one-line installer. |
...
| Run one persistent Channel conversation turn | `<AUTOMIND_CLI> converse <chat-task> --text PROMPT --user-text TEXT --agent AGENT` | `orchestrator/commands/session.py`, `orchestrator/session/conversation.py`, `orchestrator/agents.py` | Channel/session helper | model response on stdout; `conversation-turns.jsonl`; `conversation-state.json` | Front-end integration command. Reuses a read-only/no-tool primary agent session, does not append to formal `user-messages.json`, and bootstraps a recreated session from durable summary/recent turns. |
| Record a Channel/runtime observation batch | `<AUTOMIND_CLI> observe <chat-or-task-code> --json JSON` | `orchestrator/commands/session.py`, `orchestrator/observability.py`, `orchestrator/audit.py`, `orchestrator/metrics.py` | Observability helper | `audit.jsonl`, `audit.json`, `metrics.json` | Front-end/runtime integration command, not a normal user task command. Accepts only allow-listed bounded audit fields and metric names, applies existing redaction, and merges metrics safely across subprocess calls. |
| Create task and start CodeMind-owned CLI/TUI loop | `<AUTOMIND_CLI> ask "<request>" [auto\|codex\|claude\|trae] [--tui\|--detached]` | `orchestrator/main.py` | Phase 1 → 3 | `.automind/tasks/<task>/` | Starts CodeMind-owned loop. In an interactive terminal it uses the TUI-owned wrapper by default; new TUI tasks record `agentExecutionPolicy=default_bypass` without asking. Non-interactive scripts keep detached/plain behavior; if the task has no policy, Planner/Generator fall back to bypass. Planner/Generator reuse a task-local primary CLI session when the execution mode matches the task policy/fallback; model Evaluator still launches fresh isolated context and always uses the selected agent's bypass/high-automation mode. |
...
Android helper package setup is lazy/local. The public installer does not install it. If `android-preflight` or `android-probe-flow` needs `adbutils` / `uiautomator2` and they are missing, CodeMind may auto-run:

```bash
<AUTOMIND_CLI> setup-automation-tools android
```

This only creates `.venv-android-tools` in the target workspace and installs packages from the CodeMind runtime `requirements/android-tools.txt`. It does not install Android Studio, Android SDK/platform-tools, `adb`, or change the device. If setup fails, first reuse a ready CodeMind runtime helper venv when available; otherwise ask the user whether to fix Python/pip/network or package mirror/wheelhouse access, use adb-only fallback, or stop.
...
iOS helper package setup is lazy/local. The public installer does not install it. If screenshot/app-smoke evaluation needs `pymobiledevice3` and it is missing, CodeMind may auto-run:

```bash
<AUTOMIND_CLI> setup-automation-tools ios
```

This only creates `.venv-ios-tools` in the target workspace and installs packages from the CodeMind runtime `requirements/ios-tools.txt`. It does not install Xcode, change signing/keychains/profiles, trust devices, start `tunneld`, or use sudo. If setup fails, ask the user whether to fix Python/pip/network, continue without screenshot, or stop.
...
follow this decision flow when choosing CodeMind commands/scripts:
...
| Evaluate harness process | `<AUTOMIND_CLI> process-check <task-code> [--json|--soft|--no-write]` | `orchestrator/main.py`, `orchestrator/process_eval.py` | Process Evals | `process-eval.json`, `trace.json` | Checks whether CodeMind followed required process gates; complements workflow-check/completion-check. |
