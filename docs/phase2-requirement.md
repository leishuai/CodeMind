...
and ask whether AutoMind may use them, use both real device and
...
When exactly one connected real device is available, state in the review bundle that AutoMind will use that device by default for development, debugging, verification, and screenshots.
When multiple connected real devices are available, ask_user to choose the target device.
When no authorized real device is available, AutoMind should try simulator/emulator verification by default; ask_user only if no runnable simulator/emulator path exists and the remaining fallback would be static-only or otherwise sensitive.

### Real-device-default verification policy

For client/app behavior tasks AutoMind defaults to real-device verification.
...
If one real device is connected, AutoMind should announce that it will use that device by default. If multiple real devices are connected, AutoMind must ask the user to choose the device. If no real device is connected or the real device is unavailable, AutoMind should try simulator/emulator verification by default; ask_user only if no runnable simulator/emulator path exists and the remaining fallback would be static-only or otherwise sensitive.
...
unavailable signal, not as a task hard-stop by itself. AutoMind should try
...
2. command, AutoMind helper, or action sequence;
...
dependencies into AutoMind helper venvs; only Android/iOS/visual helper packages
...
For mobile App/UI tasks, do not say AutoMind cannot operate the app merely
