...
### `ask_user`
Use when user intent, product behavior, validation target, runtime/device/signing, destructive action, ambiguous/irreversible privacy/security impact, or non-obvious tradeoff needs a human decision. App-internal privacy/terms Agree/Allow/Continue and OS/app permission Allow may auto-unblock with evidence so automated verification can proceed. Use ask_user for reject/deny, login/account grant, payment/purchase/subscription, delete/reset/uninstall, external upload, signing/device trust, or ambiguous/irreversible consent. Safe close/skip/later/dismiss overlays may also auto-unblock with evidence.
