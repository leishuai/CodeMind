...
- command, AutoMind command, or action sequence;
...
Every UI action needs a post-action assertion. Do not claim AutoMind cannot operate an app just because the flow needs taps/clicks/input/navigation; use Android probe-flow, iOS XCUITest/probe-flow/action-plan, browser automation, project-native tests, logs, screenshots, and UI hierarchy where applicable.
