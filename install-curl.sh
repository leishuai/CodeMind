#!/usr/bin/env bash
#
# AutoMind public curl installer bootstrap.
#
...
log() { echo -e "${BLUE}[AutoMind bootstrap]${NC} $*"; }
warn() { echo -e "${YELLOW}[AutoMind bootstrap]${NC} $*"; }
error() { echo -e "${RED}[AutoMind bootstrap]${NC} $*" >&2; }
success() { echo -e "${GREEN}[AutoMind bootstrap]${NC} $*"; }
...
BOOTSTRAP_URL="https://raw.githubusercontent.com/leishuai/Automind/main/install-curl.sh"

usage() {
    cat <<USAGE_EOF
AutoMind curl bootstrap installer

Usage:
  curl -fsSL $BOOTSTRAP_URL | bash

What it does:
  1. clones or updates the AutoMind repo in an installer cache;
  2. syncs a git-free runtime copy into ~/.automind/automind by default;
  3. runs ~/.automind/automind/install.sh;
  4. install.sh creates the automind CLI wrapper and agent skill/command files.

Environment variables:
  AUTOMIND_REPO             Repository to clone.
                            Default: https://github.com/leishuai/Automind.git
  ...
  AUTOMIND_INSTALL_COMMAND  Whether to install /automind command. Default: 1
...
log "========================================"
log "  AutoMind public installer bootstrap"
log "========================================"
...
case "$(uname -s)" in
    Darwin|Linux)
        ;;
    *)
        warn "Untested OS: $(uname -s). AutoMind is primarily tested on macOS and Linux."
        ;;
esac
...
if [[ -d "$AUTOMIND_STAGING_HOME/.git" ]]; then
    log "Existing AutoMind installer cache found."
    ...
    if [[ "$AUTOMIND_UPDATE" == "1" ]]; then
        log "Fetching latest AutoMind ref into installer cache..."
...
log "Delegating to AutoMind install.sh..."
(
    cd "$AUTOMIND_HOME"
    bash ./install.sh
)

success "AutoMind bootstrap completed."
echo ""
echo "Try:"
