#!/usr/bin/env bash
#
# CodeAutonomy public curl installer bootstrap.
#
...
# clones or updates the CodeAutonomy git repository in a private installer cache,
# syncs a git-free runtime copy to AUTOMIND_HOME, and delegates the real install
# work to that runtime copy's install.sh.
#
# Default repository:
#   https://github.com/leishuai/CodeAutonomy.git
#
...
log() { echo -e "${BLUE}[CodeAutonomy bootstrap]${NC} $*"; }
warn() { echo -e "${YELLOW}[CodeAutonomy bootstrap]${NC} $*"; }
error() { echo -e "${RED}[CodeAutonomy bootstrap]${NC} $*" >&2; }
success() { echo -e "${GREEN}[CodeAutonomy bootstrap]${NC} $*"; }
...
BOOTSTRAP_URL="https://raw.githubusercontent.com/leishuai/CodeAutonomy/main/install-curl.sh"

usage() {
    cat <<USAGE_EOF
CodeAutonomy curl bootstrap installer

Usage:
  curl -fsSL $BOOTSTRAP_URL | bash

What it does:
  1. clones or updates the CodeAutonomy repo in an installer cache;
  2. syncs a git-free runtime copy into ~/.automind/automind by default;
  3. runs ~/.automind/automind/install.sh;
  4. install.sh creates the codeautonomy CLI plus automind compatibility alias and agent integrations.

Environment variables:
  AUTOMIND_REPO             Repository to clone.
                            Default: https://github.com/leishuai/CodeAutonomy.git
  ...
  AUTOMIND_INSTALL_COMMAND  Whether to install /codeautonomy and legacy /automind commands. Default: 1
...
log "========================================"
log "  CodeAutonomy public installer bootstrap"
log "========================================"
...
case "$(uname -s)" in
    Darwin|Linux)
        ;;
    *)
        warn "Untested OS: $(uname -s). CodeAutonomy is primarily tested on macOS and Linux."
        ;;
esac
...
if [[ -d "$AUTOMIND_STAGING_HOME/.git" ]]; then
    log "Existing CodeAutonomy installer cache found."
    ...
    if [[ "$AUTOMIND_UPDATE" == "1" ]]; then
        log "Fetching latest CodeAutonomy ref into installer cache..."
...
if command -v rsync >/dev/null 2>&1; then
    ...
    (cd "$AUTOMIND_STAGING_HOME" && tar --exclude='.git' --exclude='.automind/tasks' --exclude='.automind/summary' --exclude='dist' --exclude='.venv-*' --exclude='summaries/accumulated' -cf - .) | (cd "$AUTOMIND_HOME" && tar -xf -)
fi
...
log "Delegating to CodeAutonomy install.sh..."
(
    cd "$AUTOMIND_HOME"
    bash ./install.sh
)

success "CodeAutonomy bootstrap completed."
echo ""
echo "Try:"
