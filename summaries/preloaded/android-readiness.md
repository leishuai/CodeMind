...
- Manual action alone is not proof. If human-assisted evidence capture is used,
  CodeMind must still collect machine-checkable postcondition evidence.
...
3. Prefer the first ready helper Python in this order:
   ...
   - CodeMind runtime `.venv-android-tools` when it is already ready;
...
- reuse a ready CodeMind runtime `.venv-android-tools` if available;
