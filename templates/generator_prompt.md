...
Do not open `{task_dir}/logs/iter-{iteration}/generator-context.json` by default. It is machine/audit metadata; use the markdown context pack as the agent-facing handoff. Open the JSON only when debugging CodeMind context-pack generation itself.
...
- CodeMind runtime source, unless a framework exception is the actual blocker
...
- When switching sessions, resuming a long task, or preparing Generator -> Evaluator handoff, do not reopen the whole case file by rereading broad history, raw transcripts, full Delivery/Validation, full diffs, or CodeMind runtime source just to re-understand the task.
...
- After runtime proof already exists, do not spend another turn researching CodeMind completion internals unless a framework exception occurred. Use `workflow-check` / `completion-check` outputs as the gate contract and move directly to the next artifact or Evaluator handoff.
...
- If verification requires temporary logs inside the target iOS/Android/Web/Server project, prefix every such log line with `[CodeMind][Verify]`. Keep logs minimal/non-secret and remove or explicitly promote them before finish.
...
- If the prompt includes pending CodeMind user messages from `user-messages.json`, reconcile them with Requirements/TestCases/Plan/workflow state before coding; if they change scope or introduce risk, update artifacts and route through `ask_user`/`replan` instead of silently implementing.
...
- For Android device work, do not run raw long-lived `adb install`, `adb shell`, or logcat commands without a timeout. Prefer CodeMind wrappers such as `android-preflight`, `android-apk-probe`, and `android-probe-flow`; if a raw adb command is unavoidable, run it through a bounded Python `subprocess.run(..., timeout=...)` and record the timeout/evidence.
...
Requirements:
- Include an `CodeMind State Check` subsection with `stage=Build`, last gate status, missing artifacts if any, and next required action.
...
- If implementation required web/client/server dependency preparation, record the project-native command used or planned, the lockfile/package-manager basis, and any `dependency-check.json` path when used. Do not record CodeMind helper `.venv-*` setup as target project dependency installation.
