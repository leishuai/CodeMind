...
def get_android_tools_python() -> str:
    ...
    network), fall back to the CodeMind runtime helper venv when it is already
    ...
    if "failed to resolve" in lower or "nameresolutionerror" in lower or "temporary failure in name resolution" in lower or "could not resolve host" in lower:
        diagnostics.update({
            ...
            "suggestions": [
                ...
                "If a runtime CodeMind .venv-android-tools is already ready, let preflight reuse it instead of reinstalling project-local helpers.",
        ...
        diagnostics.update({
            ...
            "suggestions": [
                ...
                "Do not use sudo for CodeMind helper venvs; they are project-local user-space environments.",
            ],
            "triageSource": "code_deterministic",
            "needsModelReview": False,
        })
...
def _detect_python(root: Path) -> list[dict]:
    ...
    return [{
        ...
        "install": {
            ...
            "notes": ["Use the target project's documented venv name if it already has one; do not confuse it with CodeMind helper .venv-* folders."],
        },
...
def _detect_ios(root: Path) -> list[dict]:
    ...
    return [{
        ...
        "install": {
            ...
            "notes": ["CodeMind setup-automation-tools ios only installs Python helper packages; it does not install Xcode, CocoaPods, signing profiles, or trust devices."],
        },
        ...
        if not dry_run:
            ...
            if failed_steps or missing_modules:
                ...
                if item == "android":
                    ...
                    if ready_fallbacks:
                        target_report["fallbackAdvice"] = "A ready CodeMind runtime Android helper venv exists and can be reused by android-preflight/probe-flow; project-local setup can be repaired later."
                report["result"] = "fail"
            else:
