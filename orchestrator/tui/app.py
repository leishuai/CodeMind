"""Minimal CodeMind TUI timeline renderer.
...
def _runtime_status_lines(
    *,
    ...
    lines: list[str] = []
    if is_running:
        lines.append(style("Input disabled: CodeMind is running.", YELLOW, bold=True))
        ...
        lines.append(style("Input disabled: no pending question. CodeMind is ready to continue/resume.", GRAY))
        lines.append(f"{style('Phase next:', MAGENTA, bold=True)} {phase_next} / {phase_action}")
        lines.append(f"{style('Effective next:', MAGENTA, bold=True)} {effective_text}")
    return lines
...
def render_tui_snapshot(task_code: str, task_dir: Path, *, limit: int = 80, show_logo: bool = True) -> str:
    ...
    if pending:
        ...
        if pending_answer:
            ...
            prefix = style(_now_prefix(), GRAY)
            lines.append(f"{prefix} {style('Input enabled: CodeMind needs user input.', YELLOW, bold=True)}")
        ...
        if pending_answer:
            ...
            lines.append("Answer will be enabled after the current agent turn finishes. Do not answer yet; CodeMind will return to the prompt at the next safe boundary.")
    else:
...
def run_tui_interactive(task_code: str, task_dir: Path) -> None:
    ...
    while True:
        ...
        if proc.returncode != 0:
            print(f"[CodeMind TUI] command exited with code {proc.returncode}", file=sys.stderr)
        print(render_tui_snapshot(task_code, task_dir, show_logo=False))
...
def prompt_for_pending_answer(task_code: str, task_dir: Path) -> dict[str, Any] | None:
    ...
    print()
    print(style("─" * 56, GRAY))
    print(style("CodeMind needs your input", YELLOW, bold=True))
