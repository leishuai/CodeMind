"""Workflow-check and Phase 2 gate validation helpers for AutoMind.
...
def validate_planner_artifacts(task_dir: Path) -> tuple[bool, list[str]]:
    ...
    if "command" not in testcases_lower and "automind" not in testcases_lower and "命令" not in testcases:
        issues.append("TestCases.md missing executable command or AutoMind command")
...
def check_workflow_consistency(task_code: str) -> tuple[bool, dict]:
    ...
    through AutoMind phases without requirement/test drift.
    """
    ...
    if decision_bundle is None:
        ...
        # (runtimeProofRequired == "yes"), AutoMind requires either:
        ...
        if runtime_proof_required == "yes":
            if verification_target in runtime_targets:
                ...
                warnings.append(
                    ...
                    "AutoMind will accept a non-runtime verificationTarget but completion-check still requires evidence."
                )
            else:
                issues.append(
