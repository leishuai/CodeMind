"""Completion gate and verification-unblock validation for AutoMind.
...
# valid reason to interrupt the autonomous loop with ask_user. AutoMind's design
...
# config/generated wrappers (then re-running the verification once) is AutoMind's
...
# retry?" is therefore a non-whitelisted soft pause: AutoMind should just attempt
...
# NOT treated as sensitive: AutoMind is allowed to re-sign with the user's own
...
def _ask_user_is_temporary_unblock_authorization(question: dict) -> str | None:
    ...
    Such a question must not pause the loop: AutoMind should just make the
...
# trigger error, interrupt/pause, navigate, capture logcat/log) is AutoMind's own
...
# is a non-whitelisted soft pause: it delegates AutoMind's own UI-driving work
...
def _ask_user_delegates_device_operation(question: dict) -> str | None:
    ...
    AutoMind's own device-driving work back to the human, else None.
...
#
# AutoMind biases hard toward automation: for non-enterprise users, over-cautious
...
#     - "safe_self_service": AutoMind can resolve it itself (long build, minimal
...
# here either (AutoMind may re-sign with the user's own certificates / automatic
...
def validate_ask_user_category(evaluation: dict, user_intent: dict | None = None) -> tuple[list[str], list[str]]:
    ...
    if intent.get("fullAuto") is True:
        issues.append(
            ...
            "mode (preImplementationReview.fullAuto=true), so AutoMind must not interrupt "
    ...
    # `riskTier`, that self-assessment is the PRIMARY judge -- AutoMind biases
    ...
    if risk_tier is not None:
        if risk_tier == _RISK_TIER_SENSITIVE:
            ...
            return issues, warnings
        # risk_tier == safe_self_service: the model itself says AutoMind can
        ...
        if contradiction is not None:
            ...
            return issues, warnings
        issues.append(
            "ask_user rejected: askUserQuestion.riskTier=safe_self_service means AutoMind "
    ...
    if long_running_marker is not None:
        issues.append(
            ...
            "AutoMind should just run the long build/test; settle any scope/duration "
    ...
    if temp_unblock_marker is not None:
        issues.append(
            ...
            "re-running the verification once is AutoMind's own job: just attempt it, "
    ...
    if category == "real_device_or_signing":
        ...
        if delegation_marker is not None:
            issues.append(
                ...
                "interrupt/navigate) and capturing logcat/.xcresult is AutoMind's own job "
...
def normalize_evidence_assessment(item: dict) -> dict:
    ...
    AutoMind intentionally does not parse arbitrary build/runtime logs with
...
def write_completion_ledger(task_dir: Path, report: dict) -> Path:
    ...
    Historically AutoMind wrote ``VerificationLedger.json`` while leaving
