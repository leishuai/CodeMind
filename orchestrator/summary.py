...
def build_improve_suggestions(limit: int = 80) -> list[dict]:
    ...
    if len(high_conf_paths) >= 2:
        suggestions.append({
            "area": "reuse",
            "priority": "medium",
            "suggestion": "Recent runs contain high-confidence successful paths; consider promoting repeated project-local commands/patterns into Reuse.md, phase docs, or an AutoMind skill note after review.",
...
def render_improve_suggestions(limit: int = 80) -> str:
    ...
    lines = ["# AutoMind Improve Suggestions", ""]
    if not suggestions:
...
def apply_ai_knowledge_actions(task_dir: Path, ai_refinement: Optional[dict]) -> tuple[list[dict], list[str]]:
    ...
    for item in ai_refinement.get("knowledgeActions", []) or []:
        ...
        if action in {"upsert_raw", "merge_raw"}:
            ...
            header = f"\
\
<!-- AutoMind summary refiner: {datetime.now().isoformat(timespec='seconds')} task={task_dir.name} action={action} -->\
"
...
"""
    ...
    # (AutoMind runtime install). Project-bound lessons route to
...
def ensure_summary_generated(task_code: str, reason: str = "loop_terminal", ai_agent: Optional[str] = "__resolve__") -> Optional[Path]:
    ...
    try:
        ...
        print_report_manifest(task_code, heading="AutoMind reports for user review")
        return path
    except Exception as exc:
        ...
        print_report_manifest(task_code, heading="Available AutoMind reports for user review")
        return None
