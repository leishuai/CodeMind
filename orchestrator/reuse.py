"""Prompt rendering and reuse-context helpers for CodeMind.
...
def runtime_language_instruction(user_input: object) -> str:
    ...
    if language == "zh":
        return (
            ...
            "Public CodeMind artifact headings/templates may remain English when required by the workflow, but explanatory content for this task should be Chinese where practical.\n"
        )
    return (
        "## Runtime communication language\n"
...
def _parse_simple_frontmatter(text: str) -> tuple[dict, str]:
    ...
    avoids adding a YAML dependency to CodeMind.
    """
...
def accumulated_technical_summary_files(limit_files: int = 40) -> list[Path]:
    ...
    summaries in the CodeMind runtime install. They are public-safe but local to
...
def build_reuse_context(task_type: Optional[str] = None, limit_chars: int = 16_000) -> str:
    ...
    if preloaded_context:
        sections.append(
            ...
            "Path rule: `Path` is relative to the CodeMind runtime root (`$AUTOMIND_HOME` when installed, or the directory containing `automind.sh`), not the target app project or `.automind/summary/`. "
            "If the agent cwd is the app project, resolve it as `<CodeMind runtime root>/summaries/preloaded/...`.\n\n"
            + preloaded_context
        )
    ...
    if technical_context:
        sections.append(
            ...
            "(CodeMind runtime `summaries/accumulated/technical/`). Treat them as local hints only; "
    ...
    if business_context:
        sections.append(
            ...
            "These local/private project lessons exist in this CodeMind runtime checkout and are included for this machine's Reuse.md. "
