"""Compatibility phase-transition projection for CodeMind tasks.
...
def build_phase_transition_summary(task_dir: Path) -> dict[str, Any]:
    ...
    if derived.get("terminalAllowed"):
        ...
        next_owner = "human"
        reason = "runtime/effective state is waiting for user input before CodeMind can continue."
        basis = ["runtime-state nextAction=ask_user or status=human_input_pending"]
    elif evaluation.get("nextAction") == "ask_user":
        next_phase = "human_input"
        next_action = "ask_user"
        next_owner = "human"
        reason = "evaluation.json requests user input before CodeMind can continue."
