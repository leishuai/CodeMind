...
def test_export_skill_contains_skill_mode_json_handoff_contract(tmp_path: Path) -> None:
    ...
    assert "Updating CodeMind itself" in skill
    ...
    assert "name: codemind-skill" in frontmatter
...
def test_export_command_contains_json_handoff_contract(tmp_path: Path) -> None:
    ...
    command = (out_dir / "commands" / "codemind.md").read_text()
    legacy_command = (out_dir / "commands" / "automind.md").read_text()
    ...
    assert "# /codemind - CodeMind Command" in command
    assert "# /automind - CodeMind Command" in legacy_command
...
def test_export_skill_default_goes_to_downloads_with_temp_home(tmp_path: Path) -> None:
    ...
    out_dir = home / "Downloads" / "codemind-skill"
...
def test_export_skill_auto_installs_all_detected_agent_roots(tmp_path: Path) -> None:
    ...
    assert (home / ".claude" / "skills" / "codemind-skill" / "SKILL.md").exists()
    assert (home / ".codex" / "skills" / "codemind-skill" / "SKILL.md").exists()
